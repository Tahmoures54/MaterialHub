...
def home():
    ...
    if current_user.is_authenticated:
        if current_user.is_admin:
            ...
            return redirect(url_for('project_dashboard.dashboard'))  # اصلاح endpoint
        else:
...
def register():
    ...
    if form.validate_on_submit():
        ...
        if not request.form.get('accept_terms'):
            ...
            return render_template('register.html', form=form, totp_form=totp_form, qr_code=None, registered=False,
...
def reset_password_request():
    ...
    return render_template('reset_password_request.html', form=form, current_year=datetime.now().year)
...
def change_password():
    ...
    if form.validate_on_submit():
        try:
            ...
            return render_template('change_password.html', form=form, qr_code=user.qr_code_base64,
    ...
    return render_template('change_password.html', form=form, qr_code=None, totp_secret=None, current_year=datetime.now().year)
...
def terms_of_service():
    ...
    return render_template('terms_of_service.html', current_year=datetime.now().year)

@auth_bp.route('/contact_support')
def contact_support():
    """Render the contact support page."""
    return render_template('contact_support.html', current_year=datetime.now().year)